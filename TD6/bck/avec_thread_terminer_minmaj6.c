...
/* Pour la terminaison */
int _terminer = FALSE;
pthread_cond_t _c_terminer = PTHREAD_COND_INITIALIZER;
pthread_mutex_t _m_terminer = PTHREAD_MUTEX_INITIALIZER;
...
void *producteur(void* arg)  /* lit dans le fichier et ecrit dans la file _file circulaire*/
{
    ...
    do
    {
        ...
        //début terminaison     
       // pthread_mutex_lock(&_m_terminer);
        if(_terminer){
            printf("_terminer == TRUE ; on termine le thread producteur numéro %lu.  libération du mutex\n",pthread_self());
           // pthread_cond_wait(&_c_terminer,&_m_terminer);
            pthread_exit(NULL);

        }
        //pthread_mutex_unlock(&_m_terminer);
        // fin terminaison
...
/* _nbOcc == 0 pour que le consommateur continue de lire le tampon et d'alimenter le fichier toto6 jusqu'à ce que le tampon soit vide*/   
       // pthread_mutex_lock(&_m_terminer);
        while(_terminer && _nbOcc==0){
            printf("_terminer == TRUE ; on termine le thread consommateur numéro %lu.  libération du mutex\n",pthread_self());
           // pthread_cond_wait(&_c_terminer,&_m_terminer);
            printf("after wait\n");
            pthread_exit(NULL);
            printf("after exit\n");
    ...
    do
    {
            ...
            switch(choix)
            {
                ...
                case 3 : // on termine les threads producteurs qui lisent le fichier et écrit dans la file
                   ...
                   // pthread_mutex_lock(&_m_terminer);
                    _terminer=TRUE;
                   // pthread_cond_signal(&_c_terminer); // signal au consommateur 
...
int main()
{
   ...
   if ((_fd2=open("toto6", O_WRONLY|O_CREAT|O_TRUNC,0644))==-1)
   {
    ...
    // mutex condition de contrôle terminaison
    pthread_mutex_destroy(&_m_terminer);
    pthread_cond_destroy(&_c_terminer);
