...
elt lire(void)  // on lit le _file
{
    ...
    while(_suspendre){
        printf("dans fonction lire() on suspend le thread.\n");
...
int main()
{
    ...
    int fd;
    pthread_t idthreadProducteur1, idthreadProducteur2, idThreadConsommateur; /* identifiant des threads */
   ...
   if ((_fd2=open("toto6", O_WRONLY|O_CREAT|O_TRUNC,0644))==-1)
   {
    ...
    /* tâche de supervision dans le thread principal */
      // variable choix :
    int choix=0;
