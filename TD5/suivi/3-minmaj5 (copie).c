...
void *threadEcriture(void* arg)  /* lit dans le fichier et ecrit dans la file fifo circulaire*/
{
    ...
    while(1){
        ...
        if (fifo[n].taille==0) // fin de fichier
        {
...
void *convAffiche(void* arg)  /* convertit en capital le texte et l'affiche sur stdout */
{
    arg=arg;
    int i, n, ecart = 'a' - 'A';
...
// et on lui associe le descripteur sortie standard
    ...
    if (pthread_create(&idthreadEcriture1, NULL, threadEcriture, &fd)!=0)
    {
    ...
    /* creation threadEcriture1*/
    if (pthread_create(&idthreadEcriture2, NULL, threadEcriture, &fd)!=0)
    {
